...
vec3 intersect2(vec3 origin, vec3 ray, vec3 center, float r){
    ...
    if (d>r){
        ...
        return origin+t0*ray;//sqrt(origin**2+a**2*ray**2)==r**2
    }
}
...
void main()
{
    vec3 sphereCenter = vec3(sin(time), 0.5, 0.7);
    vec3 omniLight = vec3(-0.8, 1.3, -3.4);
    ...
    if (length(hit_point3)>0.){
        ...
        vec3 ray2 = omniLight-hit_point;
        //screenColor = vec4(1.,1.,0.,1.);
        screenColor = vec4((intersect(hit_point, normalize(ray2), sphereCenter, 0.4))?vec3(0,0,0):vec3(1.,1.,0.),1.0);
    } else {
        //screenColor = vec4((intersect(omniLight, -normalize(omniLight-hit_point2), sphereCenter, 0.4))?vec3(0,0,0):vec3(0,0,0.),1.0);
    }
}